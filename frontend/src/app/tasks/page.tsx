...
function TaskCard({ task, isRTL, onEdit, onDelete, onStatusChange, getPriorityColor, openDropdown, setOpenDropdown, texts, showStatus = false }: {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 transition-all duration-200 hover:shadow-md hover:border-emerald-200 dark:hover:border-gray-700 group">
            ...
            <div className="relative" data-dropdown="task-menu">
              ...
              {openDropdown === task.id && (
                <div onClick={(e) => e.stopPropagation()} className={`absolute top-full mt-1 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 py-1 min-w-[100px] z-10 ${isRTL ? 'left-0' : 'right-0'}`}>
                  <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="w-full px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-emerald-50 dark:hover:bg-gray-800 flex items-center gap-2">
