      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {filteredLeaves.length === 0 ? (
          <div className="card text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400">{texts.noLeaves}</p>
          </div>
        ) : (
                    ...
                    <div className="flex-1">
                      ...
                      {leave.user && (
                        <div className="flex items-center gap-2 mb-2">
                          <div className="w-7 h-7 bg-teal-100 dark:bg-teal-900/50 rounded-full flex items-center justify-center">
                            <span className="text-xs font-semibold text-teal-600">{leave.user.name.charAt(0)}</span>
                          </div>
                          <span className="font-medium text-gray-900 dark:text-white text-sm">{leave.user.name}</span>
                        </div>
                      )}
                    ...
                    <div className="flex items-center gap-2">
                      {(leave.userId === user?.id || (user?.role === 'owner' || user?.role === 'manager')) && leave.status === 'pending' && (
                        <button onClick={() => handleDelete(leave.id)}
                          className="flex items-center gap-1 px-3 py-2 text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 rounded-lg text-sm transition-colors"
